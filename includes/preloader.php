...
#funl-preloader-container.fade-out {
  opacity: 0;
  visibility: hidden;
  pointer-events: none;
}
#funl-preloader-container .funl-loader {
...
#funl-preloader-container .funl-loader::before {
  ...
  width: 115%; aspect-ratio: 2/1; transform: translate(-50%, -50%);
  background: radial-gradient(ellipse at center, rgba(221,45,74,.5), rgba(221,45,74,.1) 46%, transparent 70%);
  filter: blur(30px); z-index: 0;
  animation: glowbg var(--loop) ease-in-out infinite;
}
#funl-preloader-container .wordmark {
  position: relative; z-index: 1;
  font-family: "FunlPoppins", system-ui, sans-serif; font-weight: 800;
  font-size: var(--size); line-height: 1; letter-spacing: -.035em;
  display: flex; align-items: flex-end;
  white-space: nowrap; user-select: none;
}
#funl-preloader-container .ch {
  display: inline-block; transform-origin: 50% 60%;
  animation: bob var(--wave) ease-in-out infinite;
  animation-delay: calc(var(--i)*0.09s);
}
#funl-preloader-container .gl {
  display: inline-block;
  animation: pop var(--loop) ease-out infinite, charge var(--loop) ease-out infinite;
  animation-delay: calc(var(--i)*0.16s);
}
#funl-preloader-container .in {
  display: inline-block;
  -webkit-text-stroke: .062em #000; paint-order: stroke fill;
  animation: flipin .75s cubic-bezier(.2,.8,.2,1) both;
  animation-delay: calc(var(--i)*0.07s);
  transform-origin: 50% 100%;
}
#funl-preloader-container .in.g { color: var(--gray); }
#funl-preloader-container .in.r { color: var(--red); }
#funl-preloader-container .dotwrap { margin-left: .04em; }
#funl-preloader-container .in.dot {
  width: .30em; height: .30em; background: var(--red);
  border: .052em solid #000; border-radius: .04em;
  -webkit-text-stroke: 0;
}
#funl-preloader-container .fl-bar {
  position: relative; z-index: 1; width: 58%; height: 3px; border-radius: 99px;
  background: rgba(153,153,153,.18); overflow: hidden;
}
#funl-preloader-container .fl-bar::after {
...
(function() {
    ...
    // Hide loader on window load (with a small 800ms animation buffer to make sure it is seen)
    window.addEventListener("load", function() {
        setTimeout(removeLoader, 800);
    });

    // Fallback safety timeout (maximum 3 seconds)
