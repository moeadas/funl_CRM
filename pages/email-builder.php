...
if ($campaignId > 0) {
    if ($companyId) {
        $stmt = $db->prepare("SELECT content_json, name, subject FROM email_campaigns WHERE campaign_id = ? AND company_id = ?");
    ...
    if ($companyId) {
        $stmt = $db->prepare("SELECT content_json, name, subject FROM email_templates WHERE template_id = ? AND company_id = ?");
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.addEventListener('beforeunload', function (e) {
      // EmailBuilder tracks dirty state internally; if you click Save, autosave clears it.
      // Best-effort warning if user navigates away within debounce window:
